...
class WebAdminTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.log_path = str(Path(self.tempdir.name) / "test.log")
    ...
    def test_admin_pages_render_with_navigation(self) -> None:
        _app, client = self.make_test_client()
        ...
        for path, title in page_expectations:
            response = client.get(path)
            body = response.get_data(as_text=True)
            self.assertEqual(200, response.status_code)
            self.assertIn(title, body)
            self.assertIn("MTLogin", body)
            self.assertIn("nav-item active", body)

    def test_form_posts_redirect_back_to_dedicated_pages(self) -> None:
        ...
        body = response.get_data(as_text=True)
        self.assertEqual(200, response.status_code)
        self.assertIn("通知渠道已保存。", body)
        self.assertIn("通知管理", body)
        ...
        body = response.get_data(as_text=True)
        self.assertEqual(200, response.status_code)
        self.assertIn("登录账户已保存。", body)
        self.assertIn("账户管理", body)
        ...
        body = response.get_data(as_text=True)
        self.assertEqual(200, response.status_code)
        self.assertIn("管理员账号已更新。", body)
        self.assertIn("系统设置", body)

    def test_account_page_shows_latest_metrics_and_next_run(self) -> None:
        flask_app, client = self.make_test_client()
        store = flask_app.config["STORE"]
        platform = store.get_platform_by_code(web_app.DEFAULT_MT_CODE)
        account_id = store.save_account(
        ...
        self.assertEqual(200, response.status_code)
        self.assertIn("上传 10 GB", body)
        self.assertIn("下载 20 GB", body)
        self.assertIn("魔力 12345", body)
        self.assertIn("最近登录 2026-04-21 09:59:59", body)
        self.assertIn("下次执行", body)
